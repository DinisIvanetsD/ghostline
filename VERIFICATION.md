...
- `npm test`: 4 files, 26 tests passed.
- `npm run build`: passed (Vite production bundle).
- `npm run test:e2e`: 9 Playwright tests passed.
...
| Main flows end to end                    | Profile save/reload; bike add/edit/delete; trail create/import; two runs imported and compared; backup export/restore        | Pass for local-profile MVP |
| Multiple same-trail runs                 | Seven primary and two secondary synthetic runs; newly created trail receives two user-imported runs in E2E                   | Pass                       |
...
| Immediate demo                           | Nine dense synthetic runs, two trails, two bikes, one demo rider; sample GPX download                                        | Pass                       |
| No obvious broken screens/runtime errors | E2E covers analysis, history, garage, profile, trails, import, empty/corrupt state; capture records zero page errors         | Pass within tested scope   |
