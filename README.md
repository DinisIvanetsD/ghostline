...
Compare repeated runs, find lost seconds, and build a theoretical best from the fastest sectors you have actually ridden. Includes nine clearly marked synthetic runs on two Sintra-inspired trails, so the complete analysis experience works immediately.
...
1. Open **Run analysis**. The latest Pedra Branca run is compared against its automatically selected Personal Best.
...
5. Create a trail in **Trails**, or import its GPX route. Set interior sector boundaries as fractions, e.g. `0.25, 0.5, 0.75`, and name the sectors. Start and finish are included automatically.
6. **Import run** accepts a timestamped GPX descent. Use **Try a sample GPX** to download a synthetic sample, then import it against Pedra Branca or your empty new trail. The first run defines an empty trail's geometry.
...
- Trail creation, GPX route import, editable named sectors, and trail/run deletion.
- GPX track and route parsing with XML validation, namespaces, point limits, coordinate and timestamp checks.
...
- `src/lib/gpx.ts`: import boundary; untrusted files become validated points.
...
Distances use haversine calculations, speed is **km/h**, elevation/distance are **metres**, analysis time is **seconds**. GPX timestamps are epoch milliseconds; the engine also accepts epoch/relative seconds. Sector boundaries are strictly increasing interior fractions of total traveled distance. Time at a split is linearly interpolated between GPS samples. Full duration includes stationary start/end time.
...
- GPX only: one complete track/segment or route per file, up to 10 MB and 25,000 points. Runs require strictly increasing timestamps. FIT/TCX, multiple segments, trimming and sensor streams are not included.
...
2. Introduce geographic timing gates, GPS quality reporting, run trimming and FIT imports.
